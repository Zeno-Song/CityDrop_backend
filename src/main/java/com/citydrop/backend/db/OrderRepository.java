...
public interface OrderRepository
    ...
    // package physically arrives, the order joins the queue right then
    // instead of moving to AT_STATION. dropped_off_at is still recorded here
    // so it's not re-counted as eligible for the scheduler until it's
    // actually assigned a vehicle (see assignQueuedOrderAtStation).
    @Modifying
    @Query("""
    UPDATE orders
    SET status = 'QUEUED',
    ...
    // Feature 2 - CAS: QUEUED -> AT_STATION, for a queued order whose package was already dropped
    // off (droppedOffAt already set before this call -- see claimVehicleAtDropoff). Refreshes
